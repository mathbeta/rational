...
//    @Consumes(MediaType.APPLICATION_JSON)
    ...
    @POST
    @Path("docker/{op}/{hostId}")
    @Produces(MediaType.APPLICATION_JSON)
    public Message operateDocker(@ApiParam(value = "operate name", required = true) @PathParam("op") String op, @ApiParam(value = "host id", required = true) @PathParam("hostId") String hostId) {
        try {
            return this.getService().operateDocker(op, hostId);
        } catch (Exception e) {
            e.printStackTrace();
            return Message.build(e.getMessage(), false, e);
        }
