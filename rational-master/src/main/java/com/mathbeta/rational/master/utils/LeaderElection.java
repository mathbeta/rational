...
public class LeaderElection {
    ...
    private static final String leaderRoot = "/com.mathbeta.rational/master";
    private static final String name = "leader";
    private static String realName = null;
    private static Stat watchStat = null;
    ...
    private void check() {
        try {
            if (zk == null) {
                zk = new ZooKeeper(ConfigHelper.getValue("zookeeper.url"), Integer.parseInt(ConfigHelper.getValue("zk.session.timeout")), new Watcher() {
                    @Override
    ...
    private void watch(ZooKeeper zk, String root, String realName, String name) throws KeeperException, InterruptedException {
        ...
        if (watchPath == null) {
            ...
            watchStat = zk.exists(watchPath, new Watcher() {
                @Override
                public void process(WatchedEvent watchedEvent) {
