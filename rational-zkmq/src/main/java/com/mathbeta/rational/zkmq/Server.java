package com.mathbeta.rational.zkmq;
...
public class Server extends Client {
    ...
    public void listen(String queue, IListener listener) {
        try {
            ...
            PathChildrenCache cache = new PathChildrenCache(client, queueRoot, false);
            cache.start();
            cache.getListenable().addListener(new PathChildrenCacheListener() {
                ...
                public void childEvent(CuratorFramework curatorFramework, PathChildrenCacheEvent pathChildrenCacheEvent) throws Exception {
                    if (pathChildrenCacheEvent.getType() == PathChildrenCacheEvent.Type.CHILD_ADDED) {
                        ...
                        latch.await();
                        reqCache.close();
                        byte[] message = client.getData().forPath(reqPath + "/req");
                        byte[] response = listener.handle(id, message);
                        client.setData().forPath(queueRoot + "/" + id + "/res", response);
                    }
                }
